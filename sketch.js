...
function setup() {
  ...
  reset_button = createButton("restart");
  reset_button.position(width/2-150 , height/2+30)
...
function draw() {
  ...
  if (ninja.isTouching(carGroup)) {
    ...
    textSize(5)
    fill("blue")
    text("Game over , try again", width/2-150, height/2)
    ninja.changeAnimation("collision")
...
coinCount = 0
ninja.changeImage("front")
ninja.position.x = width/2
ninja.position.y = height/2
}